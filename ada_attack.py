...
flags = tf.app.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('model_dir', 'nat_pubmed', 'saved model directory')
flags.DEFINE_string('dataset', 'pubmed', 'Dataset string.')  # 'cora', 'citeseer', 'pubmed'
flags.DEFINE_integer('steps', 1000, 'Number of steps to attack')
...
flags.DEFINE_float('perturb_ratio', 0.1, 'perturb ratio of total edges.')
flags.DEFINE_float('weight_decay', 5e-4, 'Weight for L2 loss on embedding matrix.')

# Load data
adj, features,labels, y_train_ori, y_val_ori, y_test_ori, train_mask, val_mask, test_mask = load_data(FLAGS.dataset)
...
if FLAGS.method == 'CW':
    ...
    C = 150  # initial learning rate

if os.path.exists('label_' + FLAGS.dataset + "_gam"+ '.npy'):
    label = np.load('label_' + FLAGS.dataset +  "_gam"+ '.npy')
else:
...
for epoch in range(FLAGS.steps):
    ...
    B, C = sess.run([model.B, model.C], feed_dict=feed_dict)
    B_2 = B[:, np.newaxis] * B
    S = np.multiply(B_2, C)
...
x2 = sess.run(model.B, feed_dict=feed_dict)

with open("support.pkl", "wb") as s:
    pickle.dump(support, s)
sess.close()
