...
class GCN(Model):
    def __init__(self, placeholders, input_dim, bias, attack=None, **kwargs):
        ...
        self.output_dim = placeholders['labels'].get_shape().as_list()[1]
        self.placeholders = placeholders
        # self.B = tf.get_variable(name="B", initializer=self.d_sqrt_inv)
        lmd = placeholders['lmd']
        self.A = placeholders['adj']
        self.attack = attack
        self.B = tf.get_variable(name="B", initializer=bias, dtype=tf.float32)
        # self.B = tf.constant(bias, name="B", dtype=tf.float32)
        # self.B = ones(tf.shape(self.A[0])[0], name='B')
        self.B = tf.clip_by_value(self.B, clip_value_min=1e-5, clip_value_max=1e5)
        ...
        if self.attack:
            ...
            self.hat_A = [tf.cast(self.modified_A[i], dtype='float32') for i in range(self.num_supports)]
            '''
            self.rowsum = tf.reduce_sum(self.hat_A[0], axis=1) + 1e-10
            self.d_sqrt = tf.sqrt(self.rowsum)  # square root
            self.d_sqrt_inv = tf.math.reciprocal(self.d_sqrt)  # reciprocal
            self.B = tf.get_variable(name="B", initializer=self.d_sqrt_inv)
            # self.B = self.layers[]
            # self.W = tf.multiply(tf.transpose(tf.multiply(self.hat_A[0], self.d_sqrt_inv)),self.d_sqrt_inv)
            self.W = tf.multiply(tf.transpose(tf.multiply(self.hat_A[0], self.B)),self.B)
            self.d_W = tf.reduce_sum(self.W, axis=1) + 1e-10
            self.d_W_sqrt = tf.sqrt(self.d_W)
            self.d_W_sqrt_inv = tf.math.reciprocal(self.d_W_sqrt)
            self.d_W_sqrt_inv = tf.matrix_diag(self.d_W_sqrt_inv)
            self.d_W = tf.matrix_diag(self.d_W)
            self.laplacian = tf.matmul(self.d_W_sqrt_inv, tf.matmul((self.d_W - self.W), self.d_W_sqrt_inv))
            self.I = tf.eye(self.modified_A[0].get_shape().as_list()[0])
            self.support_real = 2 * self.I - self.laplacian'''

            # this self.support is a list of \tilde{A} in the paper
            # replace the 'support' in the placeholders dictionary

            self.support = self.normalize_laplacian()
            ...
            if self.attack == 'PGD':
                ...
                # self.a = self.upper_S_real[0] + lmd * self.Sgrad
            elif self.attack == 'CW':
                ...
                self.loss1 = tf.maximum(0.0, self.real - self.other + 0.)
                self.loss2 = tf.reduce_sum(self.loss1)
                self.Sgrad = tf.gradients(self.loss2, self.upper_S_real[0])
    ...
    def _attack_loss(self):
        ...
        self.attack_loss = masked_softmax_cross_entropy(self.outputs, self.placeholders['labels'],
                                                  self.placeholders['labels_mask'])
        # self.attack_loss = masked_multiclass_margin_loss(self.outputs, self.placeholders['labels'],
        #                                                 self.placeholders['labels_mask'])
    ...
    def _loss(self):
        ...
        self.loss +=  FLAGS.weight_decay * tf.nn.l2_loss(self.B)
        self.loss +=  FLAGS.weight_decay * tf.nn.l2_loss(1/self.B)

        # Cross entropy error
        self.loss += masked_softmax_cross_entropy(self.outputs, self.placeholders['labels'],
                                                  self.placeholders['labels_mask'])
        # self.loss += masked_multiclass_margin_loss(self.outputs, self.placeholders['labels'],
        #                                           self.placeholders['labels_mask'])
    ...
    def normalize_laplacian_no(self):
        if self.attack:
            W = tf.multiply(tf.transpose(tf.multiply(self.B, self.hat_A[0])), self.B)
            # d_W = tf.reduce_sum(W, axis=1) + 1e-10
            d_W = tf.clip_by_value(tf.reduce_sum(W, axis=1), clip_value_min=5e-2, clip_value_max=10)
            d_W_sqrt_inv = tf.reciprocal(tf.sqrt(d_W))
            d_W_sqrt_inv = tf.matrix_diag(d_W_sqrt_inv)
            I = tf.eye(self.hat_A[0].get_shape().as_list()[0])
            return I + tf.matmul(tf.matmul(d_W_sqrt_inv, W), d_W_sqrt_inv)
        else:
            W = tf.multiply(tf.transpose(tf.multiply(self.B, self.A[0])), self.B)
            # d_W = tf.reduce_sum(W, axis=1) + 1e-10
            d_W = tf.clip_by_value(tf.reduce_sum(W, axis=1), clip_value_min=5e-2, clip_value_max=10)
            d_W_sqrt_inv = tf.reciprocal(tf.sqrt(d_W))
            d_W_sqrt_inv = tf.matrix_diag(d_W_sqrt_inv)
            I = tf.eye(self.A[0].get_shape().as_list()[0])
            return I + tf.matmul(tf.matmul(d_W_sqrt_inv, W), d_W_sqrt_inv)
    ...
    def normalize_laplacian(self):
        if self.attack:
            ...
            W = W + I
            # d_W = tf.reduce_sum(W, axis=1) + 1e-10
            d_W = tf.reduce_sum(W, axis=1)
            d_W_sqrt_inv = tf.reciprocal(tf.sqrt(d_W))
            d_W_sqrt_inv = tf.matrix_diag(d_W_sqrt_inv)
            return tf.matmul(tf.matmul(d_W_sqrt_inv, W), d_W_sqrt_inv)
        else:
            W = tf.multiply(tf.transpose(tf.multiply(self.B, self.A[0])), self.B)
            I = tf.eye(self.A[0].get_shape().as_list()[0])
            W = W + I
